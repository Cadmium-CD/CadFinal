...
public:
    ...
    void addAP(AP tempAP){
        vAP.push_back(tempAP);
    }
};
...
public:
    ...
    vodi addNetAPC(vector<APC> netAPC){
    	arrayAPC.push_back(netAPC);
    }
};
//petal class
class panel{
...
public:
	...
	//creat grid
	vector<vector<GridPt>> gridArray;
	...
	void creatPanel(vector<RouteGuide> routeGuide,map<int, int> vertMap,map<int, int> horizMap){
		if(this->horiz){
			for(int i = 0;i<vertMap.size();i++){
				...
				for(int z = 0; z < p->rgList.size();z++ ){
					if( p->rgList.find(z) !=  p->rgList.end()){
						p->fixedWidth = p->rgList(z).y1-p->rgList(z).y0;
						break;
					}
				}
				this->addPanel(*p);
			...
			for(int i = 0;i<horizMap.size();i++){
				...
				for(int z = 0; z < p->rgList.size();z++ ){
					if( p->rgList.find(z) !=  p->rgList.end()){
						p->fixedWidth = p->rgList(z).x1-p->rgList(z).x0;
						break;
					}
				}
				this->addPanel(*p);
...
vector<AP> findGenAP(panel panel,RouteGuide rg,Recunit rec)
#endif